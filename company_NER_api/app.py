...
def run():
    ...
    class NERData(torch.utils.data.dataset.Dataset):
        def __init__(self, json_docs, NER_max_sentences=100, NER_min_char_sentence=20):
            ...
            abbreviation = ['a', 'å', 'Ǻ', 'Å', 'b', 'c', 'd', 'e', 'ɛ', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n',
                            ...
                            'abs', 'al', 'approx', 'bp', 'ca', 'cap', 'cf', 'co', 'd.p.c', 'dr', 'e.g', 'et', 'etc',
                            'er',
                            'eq',
            ...
            for doc in json_docs:
                sentences = [s for s in self.text_preprocessing(doc['text']) if len(s) > NER_min_char_sentence]
                if not sentences: continue
                # here we just sample down the number of sentences
                sentences = random.sample(sentences, min([len(sentences), NER_max_sentences]))
                for s in sentences:
                    self.example.append(
                        {'input_ids': {
                            'sentence': s,
    ...
    class NERCollator:
        ...
        def __call__(self, batch):
            tmp = self.tokenizer([i['input_ids']['sentence'] for i in batch],
            ...
            offset_mapping_from = torch.tensor([t + [-100] * (512 - len(t)) for t in from_list]).reshape(
                [-1, 512]).tolist()
            offset_mapping_to = torch.tensor([t + [-100] * (512 - len(t)) for t in to_list]).reshape([-1, 512]).tolist()
            sentences = [i['input_ids']['sentence'] for i in batch]
            urls = [i['input_ids']['url'] for i in batch]

            return [i['input_ids']['id'] for i in batch], tmp, offset_mapping_from, offset_mapping_to, sentences, urls
    ...
    class NER_model(object):
        ...
        def __init__(self, json_docs, batch_size, NER_max_sentences=100, NER_min_char_sentence=20,
            ...
            self.NER_dataloader = torch.utils.data.dataloader.DataLoader(self.NER_data,
                                                                         batch_size=self.batch_size,
                                                                         sampler=self.sampler,
                                                                         collate_fn=self.NER_collator)

        def load(self, PATH_TO_MODEL="dslim/bert-base-NER"):
            self.model = AutoModelForTokenClassification.from_pretrained(PATH_TO_MODEL).to(device)
        ...
        def infer(self):
            ...
            for batch in self.NER_dataloader:
                with torch.no_grad():
                    ids, batch, offset_mapping_from, offset_mapping_to, sentences, urls = batch
                    batch.to(device)
                    preds = self.model(**batch)
                    preds = torch.nn.functional.softmax(preds['logits'], dim=2)
                    preds = torch.argmax(preds, dim=2)

                for example_id, o, s_tokens_ids, s_attention_mask, f, t, s, url in \
                        zip(ids, preds, batch["input_ids"], batch["attention_mask"], offset_mapping_from,
                            offset_mapping_to, sentences, urls):
                    if example_id not in index_duplicate:
                        index_duplicate.add(example_id)
                    else:
                        continue

                    if self.model.config.label2id['B-ORG'] in o or self.model.config.label2id['I-ORG'] in o:

                        offset_mapping = [(from_, to_) for from_, to_ in zip(f, t) if
                                          from_ != -100 or to_ != -100]
                        ...
                        s_is_subword = []
                        for w in self.NER_collator.tokenizer.convert_ids_to_tokens(s_tokens_ids):
                            if '##' in w:
                        ...
                        for i, b_org in enumerate(B_ORG):
                            ...
                            for idx in range(b_org, max_range):
                                if o[idx] in [self.model.config.label2id['B-ORG'],
                                              self.model.config.label2id['I-ORG']] or \
                                        s_is_subword[idx]:
                            ...
                            if NE == '': continue
                            if url not in urls_NER: urls_NER[url] = {}
    ...
    ## Test with real data
    with open(os.getcwd() + '/doc_sample.json', encoding='utf-8') as f:
        OUTPUT_PAUL_API = json.load(f)[:20]


    BATCH_SIZE=64

    # NAMED ENTITY RECOGNITION MODEL: URL/TEXT-> URL/Named Entities(NE)
    NER_model = NER_model(json_docs=OUTPUT_PAUL_API, batch_size=BATCH_SIZE)
    infer = NER_model.infer()

    with open(os.getcwd() + '/doc_sample_companyNER.json', encoding='utf-8', mode='w') as f:
