...
from app.models.source import Source, Entity, Company, Topic
...
COMPANY_FP = "/resources/company/companies.csv"
COMPANY_LIST = list(pd.read_csv(COMPANY_FP)["sorted_entity"])
...
@shared_task(
    name="infer:job_company",
    bind=True,
    default_retry_delay=30,
    max_retries=3,
    soft_time_limit=10000,
)
def source_company(self, job_id, source_ids, batch_size=64):
    try:
        # Load job
        job = ~Job.get(job_id)
        ...
        sources = [
            s for s in sources if s.status != "failed" and len(s.entity) > 0
        ]
        ...
        for source in sources:
            dummy_json[source.url] = {e.label: e.count for e in source.entity}
        ...
        # Update sources with model inference results
        for source in sources:
            if source.url in infer:
                source.company = [
                    Company(label=c[0], score=c[1])
                    for c in infer[source.url].items()
                ]
                source.save()
