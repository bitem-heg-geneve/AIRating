...
from app.models.source import Entity, Source
...
@shared_task(
    name="infer:job_entity",
    bind=True,
    default_retry_delay=30,
    max_retries=3,
    soft_time_limit=10000,
)
def source_entity(self, job_id, source_ids, batch_size=64):
    try:
        # Load job
        job = ~Job.get(job_id)
        ...
        for source in sources:
            if source.url in infer:
                source.entity = [
                    Entity(label=e[0], count=e[1]) for e in infer[source.url].items()
                ]
                source.save()
        ...
        if 'job' in locals():
            job.status = "failed"
            job.save()
        # Replace with proper logging in a real environment
